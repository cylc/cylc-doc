...
All files generated by Cylc during a suite run are confined to the *workflow
...
all of this is confined to the workflow run directory too, as far as possible.
...
Confining Ouput To The Workflow Run Directory
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Output files should be confined to the workflow run directory tree. Then all
