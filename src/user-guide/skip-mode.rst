...
* It allows skipping of tasks in a running workflow using either:
  ...
  * ``cylc set --out skip`` (to immediately skip)

.. note::

   Setting :cylc:conf:`[runtime][<namespace>]run mode=skip` in your ``flow.cylc``
...
* If ``[skip]outputs`` is specified and does not include either ``succeeded``
  or ``failed``, then ``succeeded`` will still be generated.
